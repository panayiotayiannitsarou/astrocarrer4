...
_HEALTH_CAREER_RE = re.compile(
    ...
    r"οδοντίατρ\w*|κτηνίατρ\w*|μαία|μαιευτ\w*)\b",
    re.IGNORECASE,
)
...
def _career_consistency_errors(client_text: str, audit_text: str) -> list[str]:
    ...
    client_fields = _between_headings(
        client_text,
        r"Επαγγελματικ(?:οί|ούς)\s+Τομ(?:είς|έα)(?:\s+προς\s+Διερεύνηση)?",
        (r"Τελική\s+Σύνθεση", r"Σύνοψη", r"Πλαίσιο\s+Εκπαιδευτικ"),
    )
    approved_fields_norm = _career_norm(approved_fields)
    ...
    for match in re.finditer(r"Ενδεικτικά\s+επαγγέλματα\s*:\s*([^\r\n]+)", client_text, re.IGNORECASE):
